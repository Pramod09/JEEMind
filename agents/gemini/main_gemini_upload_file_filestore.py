...
common_instruction_prompt = instruction_prompt = """
    Understood ✅. The issue is that the current instruction set explicitly tells the model to **output task completion markers and formatting styles**, which then leak into user-facing responses. To fix this, we need to **internalize those markers** so they are only used for model control flow, not exposed in the final output. Here’s the corrected version:
